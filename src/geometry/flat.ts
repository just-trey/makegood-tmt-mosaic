...
import type { BaseParams, ColorSettings, FitTransform, ParsedSVG, PolyFeature, ShapeKind } from '../types';
...
export function footprintFeature(kind: ShapeKind, params: BaseParams): PolyFeature {
  let ring: Ring = [];
  if (kind === 'disc') {
    const r = (params.diameter || 0) / 2, segs = 128, pts: Ring = [];
    ...
    const w = (params.width || 0) / 2, h = (params.height || 0) / 2;
    ring = [[-w, -h], [w, -h], [w, h], [-w, h], [-w, -h]];
  } else if (kind === 'round') {
    const w = (params.width || 0) / 2, h = (params.height || 0) / 2;
    const r = Math.min(params.corner || 0, w, h), seg = 12, pts: Ring = [];
    const corners: [number, number, number, number][] = [
      [w - r, -h + r, -90, 0], [w - r, h - r, 0, 90], [-w + r, h - r, 90, 180], [-w + r, -h + r, 180, 270],
    ];
  ...
  return { type: 'Feature', properties: {}, geometry: { type: 'Polygon', coordinates: [ring] } } as PolyFeature;
}

/** Fit the SVG bbox into the footprint bbox (uniform scale, centered). */
export function fitTransform(
  svgBBox: ParsedSVG['bbox'], targetW: number, targetH: number,
  marginPct: number, scaleMult: number, offsetX: number, offsetY: number,
): FitTransform {
  ...
  const mW = targetW * (1 - marginPct / 50), mH = targetH * (1 - marginPct / 50);
...
export function transformFeature(feature: PolyFeature, fit: FitTransform): PolyFeature {
  ...
  function fixRing(ring: Ring): Ring { const m = ring.map(tp); return fit.flipY ? m.slice().reverse() : m; }
  const g = feature.geometry;
  const geom = g.type === 'Polygon'
    ? { type: 'Polygon' as const, coordinates: (g.coordinates as Ring[]).map(fixRing) }
    : { type: 'MultiPolygon' as const, coordinates: (g.coordinates as Ring[][]).map(poly => poly.map(fixRing)) };
...
export function buildGeometry(input: FlatBuildInput): FlatBuild | null {
  const { parsed, colorSettings, baseParams, shapeKind, globalDepth, recessBg, mergeGroups, baseColorHex } = input;
  if (!parsed) return null;

  const { byColor } = computeNetRegionsByColor(parsed.shapes);

  let footW: number, footH: number;
  if (shapeKind === 'disc') { footW = baseParams.diameter || 0; footH = baseParams.diameter || 0; }
  else { footW = baseParams.width || 0; footH = baseParams.height || 0; }
  ...
  interface Entry {
    color: string; key: string; members: string[]; isMerge: boolean;
    feature: PolyFeature; depth: number; isBackground?: boolean;
  }
  ...
  if (recessBg && leftover) {
    ...
    const bgDepth = (colorSettings[BACKGROUND_KEY] && colorSettings[BACKGROUND_KEY].depth) || globalDepth;
    colorEntries.push({
  ...
  const colorMeshes = colorEntries.map(c => {
    ...
    mesh.name = c.key;
    return {
      color: c.color, key: c.key, members: c.members, isMergeGroup: c.isMerge,
      depth: c.depth, mesh, area: turf.area(c.feature), areaPct: 0, isBackground: !!c.isBackground,
    } as ColorMeshEntry;
