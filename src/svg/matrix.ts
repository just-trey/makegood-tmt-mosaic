...
export const Mat = {
  identity(): Mat6 { return [1, 0, 0, 1, 0, 0]; },
  ...
  translate(tx: number, ty?: number): Mat6 { return [1, 0, 0, 1, tx, ty || 0]; },
  scale(sx: number, sy?: number): Mat6 { return [sx, 0, 0, (sy === undefined ? sx : sy), 0, 0]; },
  ...
  skewX(deg: number): Mat6 { return [1, 0, Math.tan(deg * Math.PI / 180), 1, 0, 0]; },
  skewY(deg: number): Mat6 { return [1, Math.tan(deg * Math.PI / 180), 0, 1, 0, 0]; },
};
