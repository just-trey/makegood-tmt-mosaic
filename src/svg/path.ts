...
export function parsePathD(d: string): Loop[] {
  ...
  let prevCmd: string | null = null, prevCtrl: Pt | null = null;
  const BEZ_SEGS = 18;
  while (i < tokens.length) {
    ...
    if (C === 'M') {
      ...
      const p1: Pt = prevCtrl ? { x: 2 * cx - prevCtrl.x, y: 2 * cy - prevCtrl.y } : { x: cx, y: cy };
      const p2 = { x: rel ? cx + x2 : x2, y: rel ? cy + y2 : y2 };
      const p3 = { x: rel ? cx + x : x, y: rel ? cy + y : y };
      flattenCubic({ x: cx, y: cy }, p1, p2, p3, BEZ_SEGS, cur);
      cx = p3.x; cy = p3.y; prevCtrl = p2; prevCmd = cmd;
