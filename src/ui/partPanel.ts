...
import { renderAssemblyKindSelect, renderAssemblyPartList, renderAssemblyRoleControls } from './assemblyPanel';
...
export function setShapeKind(kind: ShapeKind): void {
  ...
  $('#export-hint').innerHTML = (kind === 'assembly')
...
export function initPartPanel(): void {
  ...
  // disc
  bindShapeInput('#p-diameter', v => { state.disc.diameter = v; });
  bindShapeInput('#p-thickness', v => { state.disc.thickness = v; });
  // rect
  bindShapeInput('#p-width', v => { state.rect.width = v; });
  bindShapeInput('#p-height', v => { state.rect.height = v; });
  bindShapeInput('#p-thickness-r', v => { state.rect.thickness = v; });
  // rounded rect
  bindShapeInput('#p-width-rr', v => { state.round.width = v; });
  bindShapeInput('#p-height-rr', v => { state.round.height = v; });
  bindShapeInput('#p-corner', v => { state.round.corner = v; });
  bindShapeInput('#p-thickness-rr', v => { state.round.thickness = v; });
  // stl reference plate
  bindShapeInput('#p-width-stl', v => { state.stlPlate.width = v; });
  bindShapeInput('#p-height-stl', v => { state.stlPlate.height = v; });
  bindShapeInput('#p-thickness-stl', v => { state.stlPlate.thickness = v; });
  bindShapeInput('#p-facez', v => { state.stlPlate.faceZ = v; });
