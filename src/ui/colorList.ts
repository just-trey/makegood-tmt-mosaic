...
export function renderColorList(colorMeshes: ColorListEntry[] | null): void {
  ...
  colorMeshes.forEach(c => {
    ...
    row.querySelector<HTMLInputElement>('.depth-input')!.addEventListener('change', e => {
      state.colorSettings[c.key] = { depth: parseFloat((e.target as HTMLInputElement).value) || 0.1 };
      scheduleRebuild();
    });
    ...
    if (sel) sel.addEventListener('change', e => {
      ...
      if (t.checked) state.selectedForMerge.add(hex); else state.selectedForMerge.delete(hex);
      updateMergeButton();
    });
    ...
    if (!c.isBackground) {
      ...
      row.addEventListener('dragleave', e => { if (!row.contains(e.relatedTarget as Node)) row.classList.remove('drop-target'); });
