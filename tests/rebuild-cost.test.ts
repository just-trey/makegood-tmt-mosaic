...
// cost estimate and the last-build accessor are under test here — everything it would touch to
...
import { estimateRebuildSlow, getLastAssemblyBuild } from '../src/app/rebuild';
...
describe('estimateRebuildSlow', () => {
  it('is fast with no artwork — the bare part renders instantly', () => {
    expect(estimateRebuildSlow()).toBe(false);
  });

  it('is always slow once there is artwork — every part gets 3D CSG', () => {
    state.parsed = parsedWithPoints(3); // a triangle, as simple as artwork gets

    expect(estimateRebuildSlow()).toBe(true);
  });

  it('is cheap — it must not touch the parsed geometry', () => {
