...
/**
 * Drives the real orchestration in rebuild.ts. Only the expensive producer, buildAssemblyGeometry
 * (wasm CSG), is stubbed; the scene graph, the transform groups, the display pose and the DOM are
 * all real, because those are what this module actually decides. `npm run smoke` still covers the
 * real producer end to end in a browser.
 */
...
describe('rebuildCurrent: what every rebuild does', () => {
