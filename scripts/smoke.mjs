...
try {
  ...
  await page.waitForFunction(() => !document.querySelector('#btn-export')?.disabled, { timeout: 240_000 });
  ...
  await page.waitForFunction(() => !document.querySelector('#btn-export-stl')?.disabled, { timeout: 60_000 });
  const rows = await page.locator('#color-list .color-row').count();
  console.log('   flat rebuild done, color rows:', rows, '| tris:', await page.textContent('#stat-tris'));
  await sleep(800);
  ...
  const [dl2] = await Promise.all([page.waitForEvent('download', { timeout: 120_000 }), page.click('#btn-export')]);
  ...
  const [dl3] = await Promise.all([page.waitForEvent('download', { timeout: 120_000 }), page.click('#btn-export-stl')]);
  ...
  console.log('\nRESULT:', errors.length ? 'CONSOLE/PAGE ERRORS FOUND' : 'clean — no console or page errors');
