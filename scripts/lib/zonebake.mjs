...
/**
 * Interior loops whose isoperimetric mean width (4·area/perimeter) falls below this are folds, not
 * holes: a ribbon that doubles back on itself, enclosing perimeter but no width. Area cannot catch
...
export function procrustesFit(pairs, allowScale = true) {
  ...
  return {
    ...
    scale: s,
    // got ~= s*R*(want - pc) + qc, rearranged to got ~= s*R*want + t, so a caller can apply the fit
    // to a point it did not pair. The net layout needs exactly this and must not re-derive it.
    t: [qc[0] - s * (c * pc[0] - sn * pc[1]), qc[1] - s * (sn * pc[0] + c * pc[1])],
...
/** The seed triangles a zone grows from and the direction it grows against. */
function zoneSeed(weld, zoneCfg, patches, triGeom) {
  let seedTris;
  let growNormal;
...
/**
 * BFS from the seeded patch across shared edges, accepting triangles whose face normal stays within
 * maxAngleDeg of the config's seed direction. Returns `{ tris, growNormal }` — sorted global
 * triangle indices, and the direction they were grown against, which claimWedges splits a strip on:
 * deriving "which way does this zone face" a second time there would be a second answer.
 */
export function segmentZone(weld, zoneCfg, patches, edgeTris, triGeom) {
  ...
  return { tris: [...inZone].sort((a, b) => a - b), growNormal };
}

/**
 * Hands the strip BETWEEN two zones to them so their charts abut: an unclaimed component with
 * exactly two zones on its boundary, each triangle to the nearer grow normal, grown from each
 * zone's own edge so none joins a zone it is unconnected to. Mutates `zoneTris`. The two-zone gate
 * stops it eating the surface no zone wants, which is what `census` counts and the bake logs.
 */
export function claimWedges(weld, zoneCfgs, zoneTris, edgeTris, triGeom, growNormals) {
  ...
  for (let start = 0; start < owner.length; start++) {
    ...
    // Sorted, so which zone is "first" in the report follows the config rather than the order a
    // breadth-first walk happened to meet the two.
    const [a, b] = [...touches].sort((x, y) => x - y);
...
/**
 * Refuses a segmentation where two zones grew onto the same triangle: the runtime would cut the
 * same artwork into both charts, and the join would carry it twice. Checked before claimWedges,
 * whose first-wins `owner` would otherwise hide it.
 */
function assertNoDoubleClaim(zoneCfgs, zoneTriSets, triCount) {
  const owner = new Int32Array(triCount).fill(-1);
  const clashes = new Map();
  zoneTriSets.forEach((tris, zi) => {
    for (const ti of tris) {
      if (owner[ti] < 0) {
        owner[ti] = zi;
        continue;
      }
      const key = `"${zoneCfgs[owner[ti]].id}" and "${zoneCfgs[zi].id}"`;
      clashes.set(key, (clashes.get(key) ?? 0) + 1);
    }
  });
  if (clashes.size)
    throw new Error(
      'two zones grew onto the same triangles, so the same artwork would be cut into both ' +
        `charts and the join would carry it twice: ${[...clashes]
          .map(([k, n]) => `${k} share ${n}`)
          .join(', ')}. Lower one of their maxAngleDeg, or move a seed.`,
    );
}
...
/**
 * Isoperimetric mean width (mm), 4·area/perimeter. On a long thin rectangle this reads about twice
 * the true width, so it is not a caliper — it is the measure that tells a real narrow slot from a
 * zero-area fold, which a caliper cannot. See MIN_HOLE_WIDTH_MM.
 */
const loopMeanWidth = (pts) => {
  let p = 0;
  for (let i = 0; i < pts.length; i++) {
    const [x1, y1] = pts[i];
    const [x2, y2] = pts[(i + 1) % pts.length];
  ...
  return p > 0 ? (4 * Math.abs(loopArea(pts))) / p : 0;
};
 ...
 */
export function zoneSheetSVG(zone, kindId, chartBBox) {
  ...
  return { body, W, H, hasDead: !!deadD };
}

/**
 * The `hidden` fill pattern every sheet's dead-region overlay references, hoisted so a document
 * carrying several sheets declares the id once rather than eight times.
 */
const HIDDEN_PATTERN_DEF =
...
/** One zone's sheet as a standalone true-size template. See zoneSheetSVG for what is drawn. */
export function zoneTemplateSVG(zone, kindId, chartBBox) {
  const { body, W, H, hasDead } = zoneSheetSVG(zone, kindId, chartBBox);
...
<svg width="${W}mm" height="${H}mm" viewBox="0 0 ${W} ${H}" version="1.1"
     xmlns="http://www.w3.org/2000/svg">${hasDead ? `\n  <defs>\n${HIDDEN_PATTERN_DEF}\n  </defs>` : ''}
${body}
</svg>
`;
}
...
export function netTemplateSVG(zones, net, sheetOf, boundaries = []) {
  ...
  // name clipped off — both fenders do.
  const pad = NET_SHEET_GAP_MM;
  const x0 = round(b.minU - pad, 2);
  const y0 = round(-(b.maxV + pad), 2);
  const W = round(b.maxU - b.minU + 2 * pad, 1);
  const H = round(b.maxV - b.minV + 2 * pad, 1);
  const groups = [];
  let anyDead = false;
  for (const zone of zones) {
    const nz = net.zones[zone.id];
    const sheet = sheetOf(zone.id);
    anyDead = anyDead || sheet.hasDead;
    const th = (nz.rotationDeg * Math.PI) / 180;
    const c = Math.cos(th);
    const s = Math.sin(th);
    // Sheet coords are (u, H_sheet − v) and net coords are (u_net, −v_net); this is the one
    // composition of those two flips with the net's own rotation and offset.
    const m = [c, -s, s, c, nz.offsetU - s * sheet.H, -nz.offsetV - c * sheet.H];
    const label = nz.attached
      ? ''
      : `\n  <text x="${round(sheet.W / 2, 2)}" y="${round(Math.min(sheet.H - 4, 12) + LABEL_SIZE + 2, 2)}" ` +
        `text-anchor="middle" font-family="sans-serif" font-size="${LABEL_SIZE}" fill="${ACCENT}" ` +
        `opacity="0.75">separate sheet</text>`;
    groups.push(
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="no"?>
<!--
  Whole-part design template — true-to-size at 1:1 mm (${W} x ${H}mm).
  Every design zone laid out where it sits on the assembled part. Load it at Scale 100%,
  Offset 0/0, set to cover the whole part, and it lands on all of them at once.

  GENERATED by scripts/bake-zones.mjs - do not hand-edit; re-run the bake to regenerate.
-->
<svg width="${W}mm" height="${H}mm" viewBox="${x0} ${y0} ${W} ${H}"
     version="1.1" xmlns="http://www.w3.org/2000/svg">
  <defs>
${anyDead ? HIDDEN_PATTERN_DEF + '\n' : ''}    <pattern id="ground" width="8" height="8" patternUnits="userSpaceOnUse">
      <path d="M0 8 L8 0" stroke="${GRAY}" stroke-width="0.5" stroke-opacity="0.5"/>
    </pattern>
    <pattern id="shared" width="4" height="4" patternUnits="userSpaceOnUse">
      <path d="M0 4 L4 0 M0 0 L4 4" stroke="${ACCENT}" stroke-width="0.5" stroke-opacity="0.6"/>
    </pattern>
  </defs>
  <rect x="${x0}" y="${y0}" width="${W}" height="${H}" fill="url(#ground)"/>
${groups.join('\n')}${
    overlapD
...
/**
 * Clear space (mm) between a detached sheet and everything already on the net. Two chart snap
 * tolerances: one sheet's edge may sit CHART_SNAP_MM off its own triangulation, so a smaller gap
 * could read as two sheets touching when neither claims the surface between them.
 */
export const NET_SHEET_GAP_MM = 2 * CHART_SNAP_MM;

/**
 * `seamOf(aId, bId)` over every ordered pair of zones: `measureZoneSeam` plus the UV midpoint of
 * the seam on each side, taken over the tightest gap bucket that has any pairs at all — 2mm where
 * two zones nearly touch, 20mm where they only face each other, so a sheet is laid beside the part
 * of its neighbour it is actually nearest. Undefined for a pair no closer than the widest bucket.
 *
 * The same `measureZoneSeam` scripts/measure-zone-seams.mjs prints, so the net's attach decisions
 * re-derive from that script rather than from a second measurement.
 */
export function netSeamRelations(zones, vertsOf, sharedTolMm, weldTolMm) {
  const cap = SEAM_GAP_BUCKETS_MM[SEAM_GAP_BUCKETS_MM.length - 1];
  const pts = new Map(zones.map((z) => [z.id, zoneSeamPoints(z, vertsOf, weldTolMm)]));
  const rel = new Map();
  for (const a of zones)
    for (const b of zones) {
      if (a.id === b.id) continue;
      const A = pts.get(a.id);
      const B = pts.get(b.id);
      const m = measureZoneSeam(A, B, sharedTolMm);
      if (!m.counts[SEAM_GAP_BUCKETS_MM.length - 1]) continue;
      const near = nearestPoints(
        A.boundary.map((i) => A.pos[i]),
        B.pos,
        cap,
      );
      const tight = SEAM_GAP_BUCKETS_MM.find((t) => near.some((n) => n.j >= 0 && n.d <= t));
      const midA = [0, 0];
      const midB = [0, 0];
      let n = 0;
      near.forEach((nb, k) => {
        if (nb.j < 0 || nb.d > tight) return;
        midA[0] += A.uv[A.boundary[k]][0];
        midA[1] += A.uv[A.boundary[k]][1];
        midB[0] += B.uv[nb.j][0];
        midB[1] += B.uv[nb.j][1];
        n++;
      });
      rel.set(`${a.id}>${b.id}`, {
        m,
        tight,
        midA: [midA[0] / n, midA[1] / n],
        midB: [midB[0] / n, midB[1] / n],
      });
    }
...
function netSheetBBox(zone, theta, t) {
  ...
  const b = zone.uvBounds;
  const cs = [
    [b.minU, b.minV],
    [b.maxU, b.minV],
    [b.maxU, b.maxV],
    [b.minU, b.maxV],
  ].map(([u, v]) => [c * u - s * v + t[0], s * u + c * v + t[1]]);
  return {
    minU: Math.min(...cs.map((p) => p[0])),
    maxU: Math.max(...cs.map((p) => p[0])),
    minV: Math.min(...cs.map((p) => p[1])),
    maxV: Math.max(...cs.map((p) => p[1])),
  };
}

/**
 * The open interval of k for which `box` slid k along `d` still overlaps `other` by less than
 * `gap`, or null when no k does. Each axis contributes an interval (all of R when the direction has
 * no component on it and the boxes already overlap there), and the conflict is their intersection.
 */
function slideConflict(box, other, d, gap) {
  const axis = (bMin, bMax, oMin, oMax, dk) => {
    if (Math.abs(dk) < 1e-12)
      return bMax > oMin - gap && bMin < oMax + gap ? [-Infinity, Infinity] : null;
    const a = (oMin - gap - bMax) / dk;
    const b = (oMax + gap - bMin) / dk;
    return dk > 0 ? [a, b] : [b, a];
  };
  const iu = axis(box.minU, box.maxU, other.minU, other.maxU, d[0]);
  const iv = axis(box.minV, box.maxV, other.minV, other.maxV, d[1]);
  if (!iu || !iv) return null;
  const lo = Math.max(iu[0], iv[0]);
  const hi = Math.min(iu[1], iv[1]);
  return hi > lo ? [lo, hi] : null;
}

/**
 * Where every zone's sheet sits on the kind's net: a spanning tree over the zones, rooted at the
 * one with the most attachable neighbours.
 *
 * A neighbour sharing vertices whose rigid UV fit lands within CHART_SNAP_MM is **attached** at
 * exactly that fit (rotation and translation, never scale), so a design drawn across the join is
 * cut across it. Any other neighbour is **detached**: no rotation, so its own "up" stays vertical,
 * slid out along the side its seam with the tree faces until it clears every sheet already placed
 * by NET_SHEET_GAP_MM. Nothing is claimed about continuity there.
 *
 * `seamOf(aId, bId)` returns `measureZoneSeam(a, b)` plus the UV midpoints of the tightest bucket
 * of near pairs, or undefined for two zones that never come within the widest gap bucket.
 */
export function layoutNet(zones, seamOf, log = () => {}, warnings = []) {
  const attachable = (s) => !!(s && s.m.sharedRigid && s.m.sharedRigid.p95 <= CHART_SNAP_MM);
  const attachCount = (z) =>
    zones.filter((o) => o.id !== z.id && attachable(seamOf(z.id, o.id))).length;
  let root = zones[0];
  for (const z of zones) if (attachCount(z) > attachCount(root)) root = z;

  const placed = new Map([
    [root.id, { theta: 0, t: [0, 0], attached: true, bbox: netSheetBBox(root, 0, [0, 0]) }],
  ]);
  log(
    `net: rooted at "${root.id}" (${attachCount(root)} attachable neighbour(s)), ` +
      `sheets clear by ${NET_SHEET_GAP_MM}mm`,
  );
  // Attached first (lowest residual), then detached nearest-gap first: the tree's shape decides
  // which sheet a detached one is laid beside, so the registered joins must be settled before any
  // sheet is placed by choice.
  const rank = (s) =>
    attachable(s)
      ? [0, s.m.sharedRigid.p95, 0]
      : [1, SEAM_GAP_BUCKETS_MM.indexOf(s.tight), s.m.medianMm ?? Infinity];
  const better = (a, b) => a[0] - b[0] || a[1] - b[1] || a[2] - b[2];

  while (placed.size < zones.length) {
    let best = null;
    for (const z of zones) {
      if (placed.has(z.id)) continue;
      for (const pid of placed.keys()) {
        const s = seamOf(z.id, pid);
        if (!s) continue;
        const r = rank(s);
        if (!best || better(r, best.r) < 0) best = { zone: z, parent: pid, s, r };
      }
    }
    if (!best) {
      // Every remaining zone is out of reach of every placed one. They still take artwork, so they
      // go on the net rather than being left off it, in a row past its right edge.
      const rest = zones.filter((z) => !placed.has(z.id));
      let u = Math.max(...Array.from(placed.values(), (p) => p.bbox.maxU)) + NET_SHEET_GAP_MM;
      const minV = Math.min(...Array.from(placed.values(), (p) => p.bbox.minV));
      for (const z of rest) {
        const t = [u - z.uvBounds.minU, minV - z.uvBounds.minV];
        placed.set(z.id, { theta: 0, t, attached: false, bbox: netSheetBBox(z, 0, t) });
        u += z.uvBounds.maxU - z.uvBounds.minU + NET_SHEET_GAP_MM;
      }
      warnings.push(
        `net: ${rest.map((z) => `"${z.id}"`).join(', ')} come within ` +
          `${SEAM_GAP_BUCKETS_MM[SEAM_GAP_BUCKETS_MM.length - 1]}mm of no other zone, so ` +
          `${rest.length === 1 ? 'it is' : 'they are'} parked past the right edge of the net rather ` +
          `than beside a neighbour`,
      );
      break;
    }
    const { zone, parent, s } = best;
    const P = placed.get(parent);
    const pc = Math.cos(P.theta);
    const ps = Math.sin(P.theta);
    if (attachable(s)) {
      // The fit takes this zone's UV onto the parent's; composing with the parent's own placement
      // is what makes the tree rather than a star.
      const fit = s.m.sharedRigid;
      const th = P.theta + (fit.thetaDeg * Math.PI) / 180;
      const t = [pc * fit.t[0] - ps * fit.t[1] + P.t[0], ps * fit.t[0] + pc * fit.t[1] + P.t[1]];
      placed.set(zone.id, {
        theta: th,
        t,
        attached: true,
        parent,
        bbox: netSheetBBox(zone, th, t),
        residual: fit,
      });
      log(
        `net: "${zone.id}" ATTACHES to "${parent}" at ${fit.thetaDeg.toFixed(3)}° on ${fit.n} ` +
          `shared vertices, residual rms ${fit.rms.toFixed(3)} p95 ${fit.p95.toFixed(3)} ` +
          `max ${fit.max.toFixed(3)}mm`,
      );
    } else {
      // Out along the line from the parent's middle to the seam the two share, so a sheet lands on
      // the side of its neighbour it really touches.
      const mp = [
        pc * s.midB[0] - ps * s.midB[1] + P.t[0],
        ps * s.midB[0] + pc * s.midB[1] + P.t[1],
      ];
      const ctr = [(P.bbox.minU + P.bbox.maxU) / 2, (P.bbox.minV + P.bbox.maxV) / 2];
      let d = [mp[0] - ctr[0], mp[1] - ctr[1]];
      const dl = Math.hypot(d[0], d[1]);
      // A seam midpoint at the parent's own centre leaves no side to go out on; +u is as good as
      // any other direction and the slide below still clears everything.
      d = dl > 1e-9 ? [d[0] / dl, d[1] / dl] : [1, 0];
      const t0 = [
        mp[0] + NET_SHEET_GAP_MM * d[0] - s.midA[0],
        mp[1] + NET_SHEET_GAP_MM * d[1] - s.midA[1],
      ];
      const box = netSheetBBox(zone, 0, t0);
      const conflicts = [];
      for (const q of placed.values()) {
        const c = slideConflict(box, q.bbox, d, NET_SHEET_GAP_MM);
        if (c) conflicts.push(c);
      }
      let k = 0;
      // Smallest k >= 0 outside every conflict interval: walk the intervals in order, stepping past
      // any that still contains k.
      conflicts.sort((a, b) => a[0] - b[0]);
      for (const [lo, hi] of conflicts) if (k > lo && k < hi) k = hi;
      const t = [t0[0] + k * d[0], t0[1] + k * d[1]];
      placed.set(zone.id, {
        theta: 0,
        t,
        attached: false,
        parent,
        bbox: netSheetBBox(zone, 0, t),
      });
      const seamGap = Math.hypot(mp[0] - (t[0] + s.midA[0]), mp[1] - (t[1] + s.midA[1]));
      log(
        `net: "${zone.id}" is DETACHED, laid beside "${parent}" ` +
          `(no shared vertices${s.m.shared ? ` registering under ${CHART_SNAP_MM}mm` : ''}; ` +
          `nearest surface ${s.m.medianMm?.toFixed(1) ?? '?'}mm away). Their seams end up ` +
          `${seamGap.toFixed(1)}mm apart`,
      );
    }
  }

  const bounds = {
    minU: Math.min(...Array.from(placed.values(), (p) => p.bbox.minU)),
    minV: Math.min(...Array.from(placed.values(), (p) => p.bbox.minV)),
    maxU: Math.max(...Array.from(placed.values(), (p) => p.bbox.maxU)),
    maxV: Math.max(...Array.from(placed.values(), (p) => p.bbox.maxV)),
  };
  const out = {};
  for (const z of zones) {
    const p = placed.get(z.id);
    out[z.id] = {
      rotationDeg: round((p.theta * 180) / Math.PI, 4),
      offsetU: round(p.t[0], 4),
      offsetV: round(p.t[1], 4),
      attached: p.attached,
      ...(p.residual
        ? {
            seamResidualMm: {
              to: p.parent,
              pairs: p.residual.n,
              rms: round(p.residual.rms, 3),
              p95: round(p.residual.p95, 3),
              max: round(p.residual.max, 3),
            },
          }
        : {}),
    };
  }
  return { placed, bounds, zones: out };
}
...
function validateConfig(config) {
  ...
  if (unknown.length)
    throw new Error(
      `zone config has key(s) nothing reads: ${unknown.join(', ')}. Known keys are ` +
        `${[...CONFIG_KEYS].join(', ')}`,
    );
  ...
  for (const z of config.zones) {
    ...
    // The runtime binds a design to the whole net under this id, so a zone answering to it would
    // be picked out of the dropdown as "the whole part" and cut only itself.
    if (z.id === WHOLE_CHAIR_ZONE)
      throw new Error(`zone id "${z.id}" is reserved for a design placed on the whole part`);
    ids.add(z.id);
...
export function bakeZones(config, parts, log = () => {}, opts = {}) {
  ...
  const zones = [];
  const templates = [];
  /** Each zone's full chart bbox, so the net can redraw its sheet without re-deriving the canvas. */
  const chartBBoxes = new Map();
  // Every zone is segmented before any is unwrapped, so claimWedge can see what the angle limits
  // left between them.
  const segments = config.zones.map((z) => segmentZone(weld, z, patches, edgeTris, triGeom));
  const zoneTriSets = segments.map((s) => s.tris);
  assertNoDoubleClaim(config.zones, zoneTriSets, weld.tris.length);
  if (config.claimWedge) {
    const grow = segments.map((s) => s.growNormal);
    const { strips, skipped, census } = claimWedges(
      weld,
      config.zones,
      zoneTriSets,
      edgeTris,
  ...
  for (const [zi, zoneCfg] of config.zones.entries()) {
    ...
    // largest lobe — the sum tests/chair-zones.test.ts computes in "splits each zone into per-part
    // clip regions that together cover it"). Treat these two as the display outline, not the clip
    // region.
    ...
    for (const [pi, list] of [...byPart.entries()].sort((a, b) => a[0] - b[0])) {
      ...
      // and the bake log is where that has to be visible. Opposite direction to the island warning
    ...
    zones.push(zone);
    chartBBoxes.set(zone.id, { maxU, maxV });
    templates.push({
  ...
  // The whole kind unfolded onto one sheet. Only worth laying out where there is more than one
  // zone to lay out; a single-zone kind's net would be its own template under another name.
  let net;
  if (zones.length > 1) {
    const netVertsOf = (id) => parts.find((p) => p.libraryPartId === id).verts;
    const seamOf = netSeamRelations(
      zones,
      netVertsOf,
      sharedVertTolMm(config),
    ...
    net = {
      ...
      bounds: {
        minU: round(layout.bounds.minU, 4),
        minV: round(layout.bounds.minV, 4),
